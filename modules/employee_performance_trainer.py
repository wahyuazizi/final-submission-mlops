...
from tensorflow.keras import Input, Model, metrics, optimizers
...
def get_model(show_summary=True):
    """
    This function defines a Keras model and returns Keras object
    """
    ...
    for key, dim in CATEGORICAL_FEATURES.items():
        input_features.append(
            Input(shape=(dim + 1,), name=transformed_name(key))
        )
    ...
    concat = concatenate(input_features)
...
def run_fn(fn_args):
    ...
    tensorboard_callback = tf.keras.calbacks.TensorBoard(
